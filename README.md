# zcklllyao

> 把想法做成开源。

我关注大模型训练、AI Agent 与开发者工具，喜欢把复杂系统拆开，写成可以被更多人使用的代码。

## Open source

- [torchtitan-npu](https://github.com/zcklllyao/torchtitan-npu) — Ascend Extension for torchtitan，探索昇腾 NPU 上的大模型训练能力。
- [k_agent](https://github.com/zcklllyao/k_agent) — 面向任务执行的 Agent 实验。
- [multagent](https://github.com/zcklllyao/multagent) — 多智能体协作实验。
- [myskills](https://github.com/zcklllyao/myskills) — 个人技能与实践记录。
- [auto-fill-info](https://github.com/zcklllyao/auto-fill-info) — 自动填写信息的小工具。

## Links

[全部仓库](https://github.com/zcklllyao?tab=repositories) · [Email](mailto:zcklllyao@163.com)

---

持续学习、构建，欢迎从一个 issue 开始认识我。
